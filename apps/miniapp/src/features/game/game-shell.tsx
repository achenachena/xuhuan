  ...
  if (game.pending_scene_slug && !run) {
    ...
    screen = (
      <HubScreen
        ...
        locale={locale}
        busy={busy}
        onStart={(chapter, character, noise) =>
          void controller.startRun(chapter, character, noise)
        }
      />
    );
      ...
      case "map":
        screen = (
          <RouteMap
            ...
            busy={busy}
            onChooseNode={(nodeId) =>
              void controller.command({ type: "choose_node", node_id: nodeId })
            }
            onAbandon={() => {
...
const LoadingScreen = ({ locale }: { readonly locale: GameLocale }) => (
  <main className="grid min-h-[100dvh] place-items-center bg-[#080d18] px-6 text-center text-white">
    <div>
      <div className="mx-auto mb-5 h-12 w-12 animate-pulse rounded-full border border-cyan-300/40 bg-cyan-300/10 shadow-[0_0_28px_rgba(34,211,238,.24)]" />
      <p className="text-sm text-slate-300">{gameText(locale, "connecting")}</p>
      <p className="mt-2 font-mono text-[10px] tracking-[0.2em] text-cyan-500">
        ACTION / V2
      </p>
    </div>
  </main>
...
const FatalError = ({
  ...
  return (
    <main className="grid min-h-[100dvh] place-items-center bg-[#080d18] p-6 text-center text-white">
      <div className="max-w-xs">
        <div className="mb-4 text-5xl text-rose-400">⌁</div>
        <p className="text-sm leading-6 text-slate-300">{message}</p>
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-5 py-3 text-sm text-cyan-100"
        >
...
const ErrorToast = ({
  ...
  return (
    <button
      type="button"
      onClick={onDismiss}
      className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] left-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 rounded-xl border border-rose-400/25 bg-rose-950/95 px-4 py-3 text-left text-xs leading-5 text-rose-100 shadow-2xl"
    >
      {message}
      <span className="float-right ml-2 text-rose-400">×</span>
    </button>
  );
};
